/**
 * MP3 分片（chunked）编码器实现。
 *
 * 使用说明：
 * 1. MP3 专用 Worker blob 入口见 mp3-worker.ts
 * 2. Worker 不可用时的主线程 fallback 降级逻辑见 chunked-encoder-bridge.ts
 *
 * 请勿在 src/index.ts 中直接引入 MP3 WASM 模块，避免污染主 bundle。
 */
